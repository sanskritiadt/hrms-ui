...
function GetAllPrEngagement() {
  ...
  const handleRevenueDetailsClick = (projectId) => {
    axios
      .get(`/apigateway/hrms/engagement/getRevenueDetailsByprojectId/${projectId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      .catch((error) => {
        console.log(error);
        toast.error(error.response.data.message || "Error fetching revenue details");
      });
  };
      ...
      },
      {
        ...
        cell: (cell) => (
          <Button
            variant="contained"
            color="primary"
            onClick={() => handleRevenueDetailsClick(cell.row.original.projectId)}
          >
            View
          </Button>
        ),
